...
int main(int nvar, char** vars)
{
 ...
 int job=0;
  ...
  TH2F* fgRadius = new TH2F("fgRadius","r_{conv} distribution; r_{conv} (cm); p_{T}^{ee}",1000,0,30,1000,0,10);
   ...
   TH2F* fgTheta = new TH2F("fgTheta[%d]","#theta_{conv} distribution; #theta_{conv} (rad); p_{T}^{ee}",1000,0,TMath::Pi(),1000,0,10);
  ...
  for(int i = 0; i < dthetabin; i++)
    {
      fgmassDtheta[i] = new TH2F(Form("fgmassDtheta[%d][%d]",i),"m_{ee} distribution ;m_{ee} (GeV); p_{T}^{ee}",500,0,1,1000,0,10);
    }
  ...
  //PAIRS cuts
  float DCENTERCUT = 10;// cut the gosht
  ...
  vector<CNTE> trackcopyP;
  vector<CNTE> trackcopyN;
  int actntrkn=0,actntrkp=0,befnrealn=0,befnrealp=0;
  int nrealp = 0,nrealn=0;
  int befrealp[20],befrealn[20];
  for (int i1 = 0; i1 < nevt; ++i1) {//event from Pool1
    //cout<<"I am reading event= "<<i1<<endl;
    if(i1%1000 == 0) cout << "Event:  " << i1 << "/" << nevt << endl;
    //event->Clear();
    br->GetEntry(i1);
    actntrkp = event->GetNPtracks();
    actntrkn = event->GetNEtracks();
    ...
    if ( (actntrkp+actntrkn)==0) continue;
   
       //=========================================
    //        trk selection for evt
    //=========================================
    int ghost[20], realp[20],realn[20],real[40];
    int nghost = 0,nreal=0;
    //e+
    CNTE* track1;
    CNTE* track2;
    int ntrk=actntrkp+actntrkn;
    for (int k1 = 0; k1 < ntrk; ++k1)
      { //cout<<"in first loop k1 searching ghost, evento actual"<<endl;
	if(k1<actntrkp){
	  track1=event->GetPtrack(k1);
	}else{
	  track1=event->GetNtrack(k1-actntrkp);
	}
	int charge1=track1->GetCharge();
	if(charge1>0 && k1<actntrkp) {
	...
	for (int k2 = k1+1; k2 < ntrk; ++k2) {	  
	  //cout<<"in second loop k2 searching ghost, evento actual"<<endl;
	  if(k2<actntrkp){
	    track2=event->GetPtrack(k2);
	  }else{
	    track2=event->GetNtrack(k2-actntrkp);
	  }
	  int charge2=track2->GetCharge();
	  //if(charge1==charge2)continue;// I do not know if I need this CHECK!!
	...
	if ( ghost_flag ) {
	  ...
	  real[nreal] = k1; ++nreal;
	  if(charge1==1){realn[nrealn]=k1-actntrkp;++nrealn;}
    ...
    //======================================================
    if(i1>0){ //cout<<"inside the second loop i1>0"<<endl;
      vector <CNTE> track1cp;
      vector <CNTE> track1cn;
      vector <CNTE> track2cp;
      vector <CNTE> track2cn;
      track1cp=trackcopyP;
      track1cn=trackcopyN;
    for (int nreal1=0; nreal1 < befnrealp+befnrealn; ++nreal1)
      { //cout<<"reading evento anterior, first loop, nreal1="<<nreal1<<endl;
	if(nreal1<befnrealp){
	  //cout<<"inside loop nreal1<befnrealp "<<nreal1<<endl;
	  // track1c[nreal1]=trackcopyP[nreal1];
	  px1     = track1cp[nreal1].GetPx();
	  py1     = track1cp[nreal1].GetPy();
	  pz1     = track1cp[nreal1].GetPz();
	  pT1     = TMath::Hypot(px1, py1); // pT from std reconctruction
	  
	  phi1    = track1cp[nreal1].GetPhi();
	  zed1    = track1cp[nreal1].GetZed();
	  alpha1  = track1cp[nreal1].GetAlpha();
	  
	  arm1     = track1cp[nreal1].GetDCarm();
	  trackid1 = track1cp[nreal1].GetEMCid();   
	  charge1  = track1cp[nreal1].GetCharge();
	  
	  crkz1    = track1cp[nreal1].GetCenterZ();
	  crkphi1  = track1cp[nreal1].GetCenterPhi();  
        }else{
	  //cout<<"inside loop nreal1-befnrealp "<<nreal1<<endl;	
	  // track1c[nreal1]=trackcopyN[nreal1-befnrealp];
	  px1     = track1cn[nreal1-befnrealp].GetPx();
	  py1     = track1cn[nreal1-befnrealp].GetPy();
	  pz1     = track1cn[nreal1-befnrealp].GetPz();
	  pT1     = TMath::Hypot(px1, py1); // pT from std reconctruction
	  
	  phi1    = track1cn[nreal1-befnrealp].GetPhi();
	  zed1    = track1cn[nreal1-befnrealp].GetZed();
	  alpha1  = track1cn[nreal1-befnrealp].GetAlpha();
	  
	  arm1     = track1cn[nreal1-befnrealp].GetDCarm();
	  trackid1 = track1cn[nreal1-befnrealp].GetEMCid();   
	  charge1  = track1cn[nreal1-befnrealp].GetCharge();
	  
	  crkz1    = track1cn[nreal1-befnrealp].GetCenterZ();
	  crkphi1  = track1cn[nreal1-befnrealp].GetCenterPhi();
	}
	...
	mytrk1.SetPy(py1);
	mytrk1.SetPz(pz1);
	//cout<<"filling track 1.Print phi="<<phi1<<endl;
	fgPhiDC->Fill(phi1);
	
        track2cp=event->GetPTracks();
	track2cn=event->GetNTracks();
	for (int nreal2=0; nreal2 < nrealn+nrealp; ++nreal2)
	  { //cout<<"leyendo evento anterior,second loop, nreal2="<<nreal2<<endl;
	    if(nreal2<nrealp){
	      //cout<<"inside loop nreal2 "<<nreal2<<endl;	
	      // track2c[nreal2]=event->GetPTracks(realp[nreal2]);
	      px2     = track2cp[nreal2].GetPx();
	      py2     = track2cp[nreal2].GetPy();
	      pz2     = track2cp[nreal2].GetPz();
	      pT2     = TMath::Hypot(px2, py2);
	      
	      phi2    = track2cp[nreal2].GetPhi();
	      zed2    = track2cp[nreal2].GetZed();
	      alpha2  = track2cp[nreal2].GetAlpha();
	      
	      arm2     = track2cp[nreal2].GetDCarm();
	      trackid2 = track2cp[nreal2].GetEMCid();
	      charge2  = track2cp[nreal2].GetCharge();
       
	      crkz2    = track2cp[nreal2].GetCenterZ();
	      crkphi2  = track2cp[nreal2].GetCenterPhi();  
	    }else{
	      //cout<<"inside loop nreal2-befnrealp "<<nreal2<<endl;	
	      //track2c[nreal2]=event->GetNTracks(realn[nreal2-nrealp]);
	      px2     = track2cn[nreal2-nrealp].GetPx();
	      py2     = track2cn[nreal2-nrealp].GetPy();
	      pz2     = track2cn[nreal2-nrealp].GetPz();
	      pT2     = TMath::Hypot(px2, py2);
	      
	      phi2    = track2cn[nreal2-nrealp].GetPhi();
	      zed2    = track2cn[nreal2-nrealp].GetZed();
	      alpha2  = track2cn[nreal2-nrealp].GetAlpha();
	      
	      arm2     = track2cn[nreal2-nrealp].GetDCarm();
	      trackid2 = track2cn[nreal2-nrealp].GetEMCid();
	      charge2  = track2cn[nreal2-nrealp].GetCharge();
	      
	      crkz2    = track2cn[nreal2-nrealp].GetCenterZ();
	      crkphi2  = track2cn[nreal2-nrealp].GetCenterPhi();
	    }
    ...
    }//if to check trackcopy fill or empty
    for(int i=0;i<20;i++){befrealp[i]=0;befrealn[i]=0;}
    trackcopyP.clear();
    trackcopyN.clear();
    trackcopyP=event->GetPTracks();
    trackcopyN=event->GetNTracks();
    for(int ip=0;ip<nrealp;++ip)//the cuts are already done in the first loop
      { //cout<<"making copy event Ptrack "<<ip<<endl;
	//trackcopyP[ip]=event->GetPtrack(realp[ip]);
	trackcopyP[ip].SetPx(trackcopyP[ip].GetPx());
	trackcopyP[ip].SetPy(trackcopyP[ip].GetPy());
	trackcopyP[ip].SetAlpha(trackcopyP[ip].GetAlpha());
	trackcopyP[ip].SetPhi(trackcopyP[ip].GetPhi());
	trackcopyP[ip].SetZed(trackcopyP[ip].GetZed());
	trackcopyP[ip].SetPz(trackcopyP[ip].GetPz());
	trackcopyP[ip].SetCharge(trackcopyP[ip].GetCharge());
	trackcopyP[ip].SetDCarm(trackcopyP[ip].GetDCarm());
	trackcopyP[ip].SetEMCid(trackcopyP[ip].GetEMCid());
	befrealp[ip]=realp[ip];
      }
    for(int in=0;in<nrealn;++in)
      { //cout<<"making copy event Ntrack "<<in<<endl;
	//trackcopyN[in]=event->GetNtrack(realn[in]);
	trackcopyN[in].SetPx(trackcopyN[in].GetPx());
	trackcopyN[in].SetPy(trackcopyN[in].GetPy());
	trackcopyN[in].SetAlpha(trackcopyN[in].GetAlpha());
	trackcopyN[in].SetPhi(trackcopyN[in].GetPhi());
	trackcopyN[in].SetZed(trackcopyN[in].GetZed());
	trackcopyN[in].SetPz(trackcopyN[in].GetPz());
	trackcopyN[in].SetCharge(trackcopyN[in].GetCharge());
	trackcopyN[in].SetDCarm(trackcopyN[in].GetDCarm());
	trackcopyN[in].SetEMCid(trackcopyN[in].GetEMCid());
	befrealn[in]=realn[in];
      }
    befnrealn=nrealn;
    befnrealp=nrealp;
    nrealp=0;nrealn=0;
  ...
  cout<<"writing bg histograms to file"<<endl;
  //TFile* f2 = new TFile(Form("output_FG/mixingbg_%d.root",job),"RECREATE");
...
fgPhiv->Write("fgPhiv");
fgRadius->Write("fgRadius");
fgPhi->Write("fgPhi");
fgTheta->Write("fgTheta");
fgDz->Write("fgDz");
fgDtheta->Write("fgDtheta");
fgDphi->Write("fgDphi");
  ...
  for (int i = 0; i < cutbin; ++i)
  {
    ...
    fgmassCut[i]->Write(Form("fgmassCut%d_%d", i)); // 2D hist
  }
  ...
  for(int i = 0; i < rbin1; i++)
  {
    fgmassRadius1[i]->Write(Form("fgmassRadius1%d", i));
  }
  for(int i = 0; i < rbin2; i++)
  {
